...
part_dict = {11: 'e-',-11: 'e+',13: 'mu-',-13: 'mu+', 22: 'photon',211: 'pi+',-211: 'pi-', 2212:'p+',-2122: 'p-'} 
...
def list_pt(filename, treeNum, eventNum, cutoff=0):
	...
	for event in tNum:
		if (i >= eventNum): break
		for j in range(len(event.genjetid)):
			if (event.genjetid[j] == 15): #only examine tau jets
				print('Event ' + str(i) + ' jet ' + str(j) + ' PT ' + str(event.genjetpt[j]))
				for k in range(len(event.genindex)):
					if (event.genindex[k] == j and event.genpt[k] > cutoff):
						ID = event.genid[k]
						print('\t PT ' + str(event.genpt[k]) +' ID '+ part_dict.get(ID, str(ID)))
		i += 1
...
for var in varlist:
    distplot(filename2,"GenNtupler/gentree",var[0],var[3],var[4],var[5],"1"+barcut,var[6],var[1],var[2]+prefix2+"_Barrel",var[1]+" Distribution Barrel"+prename2)
    distplot(filename2,"GenNtupler/gentree",var[0],var[3],var[4],var[5],"1"+endcut,var[6],var[1],var[2]+prefix2+"_Endcap",var[1]+" Distribution Endcap"+prename2)
    compplot(filename2,"GenNtupler/gentree",var[0],var[3],var[4],var[5],var[6],var[1],var[2]+prefix2+"_Barrel",var[1]+" Comparison Barrel"+prename2,"(abs(genjetid)<6 || abs(genjetid)==21)"+barcut,"Q/G Jet","abs(genjetid)==15"+barcut,"Tau","(abs(genjetid)>=6 && abs(genjetid)!=21 && abs(genjetid)!=15)"+barcut,"Other")
    compplot(filename2,"GenNtupler/gentree",var[0],var[3],var[4],var[5],var[6],var[1],var[2]+prefix2+"_Endcap",var[1]+" Comparison Endcap"+prename2,"(abs(genjetid)<6 || abs(genjetid)==21)"+endcut,"Q/G Jet","abs(genjetid)==15"+endcut,"Tau","(abs(genjetid)>=6 && abs(genjetid)!=21 && abs(genjetid)!=15)"+endcut,"Other")


#list_particles(file0, "GenNtupler/gentree", 5)
#list_pt(filename1, "GenNtupler/gentree", 50, 0)
